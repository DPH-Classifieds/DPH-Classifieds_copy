...
def test_admin_upgrade_requests_does_not_query_featured_table():
    ...
    m = re.search(r"def admin_list_listing_upgrade_requests\(.*?\n(?=\n@|\ndef )", src, re.S)
...
def test_signup_email_block_is_wrapped_in_try_except():
    """The dealer_signup_admin_notification call must not crash the submit flow."""
    src = _read_app()
    # Find the call to _send_dealer_signup_admin_notification
    idx = src.find("_send_dealer_signup_admin_notification(")
    assert idx > 0, "signup email call not found"
    # Look back 200 chars for the nearest "try:"
    window = src[max(0, idx - 500):idx]
    assert "try:" in window, (
        f"signup email call must be inside a try block; "
        f"context before: {window[-200:]!r}"
    )
    # Look forward for an except handler
    forward = src[idx:idx + 800]
    assert "except" in forward, "no except handler after signup email call"
...
def test_approved_email_block_is_wrapped_in_try_except():
    ...
    import re as _re
    matches = [m.start() for m in _re.finditer(
...
def test_500_hints_for_missing_tables():
    ...
    src = _read_app()
    assert "is missing. Run migrations/2026_08_18_dealer_listing_upgrade_requests.sql" in src
...
def test_auto_approval_uses_lazy_imports():
    ...
    src = _read_app()
    m = re.search(r"def _schedule_dealer_auto_approval_if_eligible\(.*?\n(?=\n\ndef )", src, re.S)
...
def test_email_helpers_all_have_resend_disabled_branch():
    ...
    for fn in (
        ...
        assert "RESEND_FROM_EMAIL" in src, f"{fn.__name__} doesn't check RESEND_FROM_EMAIL"
        assert "Missing RESEND_FROM_EMAIL" in src, f"{fn.__name__} doesn't return None on missing key"
