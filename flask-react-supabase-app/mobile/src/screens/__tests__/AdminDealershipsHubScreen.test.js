...
jest.mock('../admin/AdminDealersScreen', () => {
  const React = require('react');
  const { View } = require('react-native');
  return { __esModule: true, default: () => React.createElement(View, null) };
});
jest.mock('../admin/AdminDealerUpgradeRequestsScreen', () => {
  const React = require('react');
  const { View } = require('react-native');
  return { __esModule: true, default: () => React.createElement(View, null) };
});
...
const AdminDealershipsHubScreen = require('../admin/AdminDealershipsHubScreen').default;
...
test('AdminDealershipsHubScreen renders LIGHT_COLORS values when theme is light', async () => {
  mockTheme = 'light';
...
test('AdminDealershipsHubScreen renders DARK_COLORS values when theme is dark', async () => {
  mockTheme = 'dark';
