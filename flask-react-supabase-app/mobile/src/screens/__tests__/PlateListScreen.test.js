...
jest.mock('../../utils/apiClient', () => ({
  __esModule: true,
  default: { get: jest.fn().mockResolvedValue({ plates: [] }) },
}));
...
const PlateListScreen = require('../listing/PlateListScreen').default;
...
test('PlateListScreen renders LIGHT_COLORS values when theme is light', () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON, findByText } = render(<PlateListScreen navigation={navigation} />);
...
test('PlateListScreen renders DARK_COLORS values when theme is dark', () => {
  mockTheme = 'dark';
