...
jest.mock('../../utils/apiClient', () => ({
  __esModule: true,
  default: {
    get: jest.fn().mockResolvedValue({ dealership: { id: 'd1', name: 'Test Motors', status: 'active', members: [] } }),
  },
}));
...
const AdminDealershipDetailScreen = require('../admin/AdminDealershipDetailScreen').default;

const route = { params: { dealershipId: 'd1' } };
...
test('AdminDealershipDetailScreen renders LIGHT_COLORS values when theme is light', async () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON, findByText } = render(<AdminDealershipDetailScreen route={route} navigation={navigation} />);
...
test('AdminDealershipDetailScreen renders DARK_COLORS values when theme is dark', async () => {
  mockTheme = 'dark';
  ...
  expect(tree).toContain('168,180,172');
  expect(tree).not.toContain('0E1512');
  expect(tree).not.toContain('0B6B4C');
  expect(tree).not.toContain('91,101,95');
});
