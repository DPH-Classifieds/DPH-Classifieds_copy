...
jest.mock('../../context/AuthContext', () => ({
  useAuth: () => ({ signUp: jest.fn(), signInWithGoogle: jest.fn() }),
}));
jest.mock('../../utils/apiClient', () => ({
  __esModule: true,
  default: { get: jest.fn().mockResolvedValue(null), post: jest.fn().mockResolvedValue({}) },
}));
jest.mock('../../utils/turnstile', () => ({ TurnstileModal: () => null }));
...
const SignupScreen = require('../auth/SignupScreen').default;
...
test('SignupScreen renders LIGHT_COLORS background and accent when theme is light', () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON, findByText } = render(<SignupScreen navigation={navigation} />);
  return findByText('Create your account').then(() => {
    const tree = flattenStyles(toJSON());
    // LIGHT_COLORS.background = '#FAFAFA' (page base — unique to LIGHT)
    // LIGHT_COLORS.accent    = '#0B6B4C' (DPH logo + footer link — unique to LIGHT)
...
test('SignupScreen renders DARK_COLORS background and accent when theme is dark', () => {
  mockTheme = 'dark';
