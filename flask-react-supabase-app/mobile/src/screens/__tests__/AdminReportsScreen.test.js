...
const AdminReportsScreen = require('../admin/AdminReportsScreen').default;
...
test('AdminReportsScreen renders LIGHT_COLORS values when theme is light', async () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON } = render(<AdminReportsScreen />);
  await waitFor(() => {
    const tree = flattenStyles(toJSON());
    return tree.includes('FFFFFF');
  });
  const tree = flattenStyles(toJSON());
...
test('AdminReportsScreen renders DARK_COLORS values when theme is dark', async () => {
  mockTheme = 'dark';
  mockColors = DARK_COLORS;
  const { toJSON } = render(<AdminReportsScreen />);
  await waitFor(() => {
    const tree = flattenStyles(toJSON());
    return tree.includes('0C1C13');
  });
  const tree = flattenStyles(toJSON());
