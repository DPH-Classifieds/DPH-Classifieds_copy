...
jest.mock('../../utils/apiClient', () => ({
  __esModule: true,
  default: { get: jest.fn().mockResolvedValue({ parts: [] }) },
}));
...
const PartListScreen = require('../listing/PartListScreen').default;
...
test('PartListScreen renders LIGHT_COLORS values when theme is light', () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON, findByText } = render(<PartListScreen navigation={navigation} />);
...
test('PartListScreen renders DARK_COLORS values when theme is dark', () => {
  mockTheme = 'dark';
