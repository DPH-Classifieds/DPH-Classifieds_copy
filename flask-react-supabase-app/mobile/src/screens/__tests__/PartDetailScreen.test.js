...
const PartDetailScreen = require('../listing/PartDetailScreen').default;

const route = { params: { listingId: 'test-part-1', listing: { id: 'test-part-1', part_type: 'Brake Pad', price: 250, brand: 'Bosch', seller_name: 'Test Seller', city: 'Dubai' } } };
...
test('PartDetailScreen renders LIGHT_COLORS values when theme is light', async () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON, findByText } = render(<PartDetailScreen route={route} navigation={navigation} />);
...
test('PartDetailScreen renders DARK_COLORS values when theme is dark', async () => {
  mockTheme = 'dark';
