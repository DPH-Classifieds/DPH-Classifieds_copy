...
jest.mock('../../utils/apiClient', () => ({
  __esModule: true,
  default: {
    get: jest.fn().mockResolvedValue({ leads: [], total: 0 }),
  },
}));
...
const DealerLeadsScreen = require('../dealer/DealerLeadsScreen').default;
...
test('DealerLeadsScreen renders LIGHT_COLORS values when theme is light', async () => {
  mockTheme = 'light';
  mockColors = LIGHT_COLORS;
  const { toJSON, findByText } = render(<DealerLeadsScreen navigation={navigation} />);
...
test('DealerLeadsScreen renders DARK_COLORS values when theme is dark', async () => {
  mockTheme = 'dark';
