...
export default function VinRevealAnalyticsModal({ open, onClose, listings = [], days }) {
  ...
  useEffect(() => { if (!open) { setSelected(null); setActors([]); } }, [open]);
  if (!open) return null;
  const inspect = async (listing) => {
    setSelected(listing); setLoading(true); setActors([]);
    try {
