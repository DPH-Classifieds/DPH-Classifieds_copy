...
describe("OTPVerification send confirmation", () => {
  afterEach(() => { jest.restoreAllMocks(); });

  test("a prefilled number is shown but never receives a code until the user confirms", async () => {
    const fetchMock = jest.fn();
    global.fetch = fetchMock;

    render(<OTPVerification mode="page" phone="+971501234567" purpose="profile_verify" />);
    ...
    expect(screen.getByRole("button", { name: /send otp to/i })).toBeEnabled();
  });
  ...
  test("code inputs stay hidden until a code is sent, then appear (two-step UX)", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /send otp to/i }));
  ...
  test("Send is disabled and never fires for a partial number, enabled once complete", async () => {
    ...
    render(<OTPVerification mode="page" purpose="profile_verify" />);

    const phoneField = screen.getByRole("textbox");
    const sendBtn = screen.getByRole("button", { name: /send otp to/i });
  ...
  test("page mode renders a single centered card (no nested .auth-card wrapper)", () => {
    ...
    const { container } = render(
      <OTPVerification mode="page" purpose="profile_verify" />
    );
