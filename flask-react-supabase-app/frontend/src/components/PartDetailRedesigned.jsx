  ...
  const trackLeadEvent = async (action) => {
    ...
    try {
      ...
      await fetch(`${API_URL}/api/listings/part/${id}/lead-events`, {
        ...
        body: JSON.stringify({ action, source: 'part_detail', payload: { listing_id: id }, metadata: getBotSignals(), ...getWebAnalyticsIdentity() }),
        keepalive: true,
      });
    } catch (error) {
  ...
  return (
    <div className="cd-container">
      ...
      <div className="cd-max-width">
        ...
        <RecommendedListings listingType="part" listingId={id} limit={6} />
      </div>
      {lightboxOpen && (
