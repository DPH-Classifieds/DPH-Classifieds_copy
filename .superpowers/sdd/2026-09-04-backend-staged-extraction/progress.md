...
Task 8r car update extraction: implementation verified (moved the authenticated car JSON/multipart update route and both legacy URL registrations into `routes/car_update.py`; preserved ownership, field validation, VIN/phone/WhatsApp gates, image replacement/primary-image behavior, lifecycle/price notifications, direct-call compatibility, and manifest ownership). Existing direct-upload/media suite `66 passed` before manifest hash refresh; `app.py` is now `20,494` lines. Full backend, Docker, frontend/mobile, and public browser verification must be rerun at this head.
